...
export default function App() {
  ...
  const Lstraight = Math.max(
    0,
    (Ptarget - 2 * Math.PI * Rtrack) / 2,
  );
  ...
  // -----------------------
  // SQUARE-FIT (tilt without changing geometry)
  // -----------------------
  // Tilt angle (degrees). Use 90 for "rotate portrait/landscape swap".
  // You can change this to 45 etc. and it still fits the square.
  const TILT_DEG = -30;

  // Make outer SVG square
  const S = Math.max(viewW, viewH);

  // Compute bounding box of the rotated rectangle (viewW x viewH)
  // Then scale so it fits inside S x S.
  const theta = (TILT_DEG * Math.PI) / 180;
  const c = Math.abs(Math.cos(theta));
  const s = Math.abs(Math.sin(theta));
  const rotW = viewW * c + viewH * s;
  const rotH = viewW * s + viewH * c;
  const fitScale = Math.min(S / rotW, S / rotH);

  const cx = viewW / 2;
  const cy = viewH / 2;

  // Transform pipeline:
  // 1) move original content center to origin
  // 2) rotate + scale
  // 3) move into the center of the square viewport
  const contentTransform = `translate(${S / 2} ${S / 2}) rotate(${TILT_DEG}) scale(${fitScale}) translate(${-cx} ${-cy})`;
  ...
  function hingeOnTrackByS(s) {
    const b = pillBoundaryByArcLength(
      s,
      trackX,
      trackY,
      trackW,
  ...
  const [renderBlades, setRenderBlades] = useState(
    () => bladesRef.current,
  );
